...
public class UserService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String email) throws UsernameNotFoundException {
        ...
        User user = userRepository.findUserByEmail(email).orElseThrow(() -> new RuntimeException("Customer not found with id: " + email));
...
//    @Transactional
//    public SearchUser getRandomUserByHobbyNamesOrParentId(User user) {
//        if (user == null || user.getHobbies() == null || user.getHobbies().isEmpty()) {
//            throw new IllegalArgumentException("User hobbies cannot be empty");
//        }
//
//        // Extract hobby names from the user object
//        List<String> hobbyNames = user.getHobbies()
//                .stream()
//                .map(hobby -> hobby.getName())
//                .collect(Collectors.toList());
//
//        List<Byte> parentIds = user.getHobbies()
//                .stream()
//                .map(hobby -> hobby.getParentId())
//                .filter(Objects::nonNull) // Avoid null values
//                .map(Long::byteValue) // Convert Long to Byte
//                .collect(Collectors.toList());
//
//        SearchUser searchUser = new SearchUser(userRepository.findRandomUserByHobbyNamesOrParentId(hobbyNames,parentIds));
//        searchUser.setAge(LocalDate.now().minusYears(userRepository.findRandomUserByHobbyNamesOrParentId(hobbyNames, parentIds).getBirthDate().getYear()).getYear());
//        return searchUser;
//    }

//    @Transactional
//    public List<Object> findUsers(User user){
//
////        if (user == null || userRepository.findUserByUserName(user.getUserName()).get().getHobbies() == null || userRepository.findUserByUserName(user.getUserName()).get().getHobbies().isEmpty()) {
////            throw new IllegalArgumentException("User hobbies cannot be empty");
////        }
////        return userRepository.findUsers(user.getUserName());
//        return userRepository.findUsers();
//    }

    public Page<User> findUsers(User user, int offset, int pageSize) {
        ...
        return userRepository.findUsers(user.getUserName(), pageable);
    }
}
